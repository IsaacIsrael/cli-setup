...
Structure: an index page (`commands.md`) covering the global flags and a short list of planned commands, plus **one page per implemented command** under `commands/<cmd>.md`. Add a command's own page when it lands; until then it stays as a planned entry on the index.
...
Checklist: entries match the code in [`src/bin/cli-setup`](../../../src/bin/cli-setup) and its specs; implemented behavior is documented as working and planned commands are clearly marked not-yet-implemented; exit codes stated.
